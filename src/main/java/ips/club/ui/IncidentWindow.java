...
public class IncidentWindow extends JFrame {
    ...
    private void initUI() {
        ...
        gbc.gridx = 1; form.add(cbType, gbc); y++;

        cbLocation = new JComboBox<>();
        cbLocation.setEnabled(false);
        gbc.gridx = 0; gbc.gridy = y; form.add(new JLabel("Localización:"), gbc);
        gbc.gridx = 1; form.add(cbLocation, gbc); y++;

        txtComment = new JTextArea(8, 20);
        txtComment.setLineWrap(true);
        txtComment.setWrapStyleWord(true);
        gbc.gridx = 0; gbc.gridy = y; form.add(new JLabel("Descripción:"), gbc);
        gbc.gridx = 1; form.add(new JScrollPane(txtComment), gbc); y++;
        ...
        cbType.addItemListener(e -> {
            if (e.getStateChange() == ItemEvent.SELECTED) {
                ...
                if (!isInstalaciones) cbLocation.setSelectedItem(null);
            }
        });
        ...
        btnList.addActionListener(e -> {
            IncidentsListWindow listWindow = new IncidentsListWindow(incController, currentUser);
    ...
    private void loadTypes() {
        try {
            ...
            for (IncidentType t : types) cbType.addItem(t);
        } catch (Exception ex) {
             JOptionPane.showMessageDialog(this, "No se pudieron cargar los tipos de Incidencias ", "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void loadLocations() {
        try {
            ...
            for (Location l : locs) cbLocation.addItem(l);
            cbLocation.setSelectedItem(null);
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this, "No se pudieron cargar las localizaciones", "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void onCreate() {
        try {
            ...
            if (t == null) {
                JOptionPane.showMessageDialog(this, "Selecciona un tipo de incidencia.", "Validación", JOptionPane.WARNING_MESSAGE);
                return;
            }
            ...
            if (isInstalaciones && (loc  == null)) {
                JOptionPane.showMessageDialog(this, "La localizacion no puede estar vacía.", "Validación", JOptionPane.WARNING_MESSAGE);
                return;
            }
            ...
            JOptionPane.showMessageDialog(this,("Incidencia creada. ID = " + newId),"OK",JOptionPane.INFORMATION_MESSAGE);
            clearForm();
    ...
    private void clearForm() {
        ...
        cbLocation.setSelectedItem(null);
        cbLocation.setEnabled(TYPE_ACTIVA_LOCALIZACION.equalsIgnoreCase(cbType.getSelectedItem() != null ? cbType.getSelectedItem().toString() : ""));
        txtComment.setText("");
    }
}
